...
def run():
    ...
    for item in pend_an:
        ...
        try:
            ...
            # 🟢 Markdown 源码卡片 (会在发送时被转换成漂亮 HTML)
            card = f"""
### {disp} {badge}
> **{tt}**
...
{ans}
            ...
            if not first_sent:
                ...
                # 注意：这里直接传 card，send_mail 函数内部会负责美化
    ...
    # 4. 发送
    if reports:
        if len(reports) == 1 and first_sent: pass # 避免重复
        else:
            zips = []
            cz, csz = [], 0
            for f in atts:
