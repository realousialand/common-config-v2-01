...
# --- 🛠️ 1. 核心配置区 (环境变量优先) ---
# 请确保在系统环境变量中设置了这些值，或者直接在这里修改默认值
LLM_API_KEY = os.environ.get("LLM_API_KEY", "你的API_KEY")
LLM_BASE_URL = "https://api.siliconflow.cn/v1"
LLM_MODEL_NAME = os.environ.get("LLM_MODEL_NAME", "deepseek-ai/DeepSeek-R1-distill-llama-70b") # 建议用 V3 或 R1

EMAIL_USER = os.environ.get("EMAIL_USER", "你的邮箱@gmail.com")
EMAIL_PASS = os.environ.get("EMAIL_PASS", "你的应用专用密码")
...
TARGET_SUBJECTS = [
    ...
    "new research", "Stork", "ScienceDirect", "Chinese politics", "Imperial history"
]

# 本地文件设置
HISTORY_FILE = "data/history.json"
DOWNLOAD_DIR = "downloads"
MAX_ATTACHMENT_SIZE = 19 * 1024 * 1024  # 19MB (留出余量)
    ...
    if source_data.get("url") and source_data["url"].endswith(".pdf"):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                # 使用 pymupdf4llm 提取为 Markdown (包含表格识别)
...
def analyze_with_llm(content, content_type, source_url=""):
    """
    LLM 分析函数 - 已修复 Visual Enhancement Prompt
    """
    prompt = f"""
...
def get_unique_id(source_data):
    if source_data.get("id"): return source_data["id"]
    elif source_data.get("url"): return hashlib.md5(source_data["url"].encode()).hexdigest()
    return None
